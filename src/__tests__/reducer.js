...
describe('reducer', () => {
  ...
  it('UPDATE_FILTER string payload', () => {
    ...
    const state = reducer(initialState, {
      ...
      meta: { name: 'test', prop: 'abc', kind: kinds.multiple },
      payload: 'TEST'
    });
    expect(state).toEqual(
      ImmutableMap({
  ...
  it('UPDATE_FILTER value', () => {
    ...
    const state = reducer(initialState, {
      type: actionTypes.UPDATE_FILTER,
      meta: { name: 'test', prop: 'abc', kind: kinds.value },
      payload: 'TEST'
    });
    expect(state).toEqual(
      ImmutableMap({
        test: ImmutableMap({
          abc: { negated: false, operator: OR, prop: 'abc', value: ['TEST'] }
        })
      })
    );
  });
  ...
  it('UPDATE_FILTER range', () => {
    ...
    const state = reducer(initialState, {
      type: actionTypes.UPDATE_FILTER,
      meta: { name: 'test', prop: 'abc', kind: kinds.range },
      payload: { min: 0, max: 1 }
    });
    expect(state).toEqual(
      ImmutableMap({
  ...
  it('UPDATE_FILTER range throw on bad value', () => {
    ...
    ).toThrow(/range value must have a min and a max/);
    expect(() =>
      reducer(initialState, {
        type: actionTypes.UPDATE_FILTER,
        meta: { name: 'test', prop: 'abc', kind: kinds.range },
    ...
    ).toThrow(/range min must be smaller than range max/);
  });
  ...
  it('UPDATE_FILTER range throw on unexpected kind', () => {
    ...
    expect(() =>
      reducer(initialState, {
        type: actionTypes.UPDATE_FILTER,
        meta: { name: 'test', prop: 'abc', kind: 'bad' },
        payload: { max: 1 }
      })
    ).toThrow(/unexpected kind: bad/);
  });
  ...
  it('SET_NEGATED empty', () => {
    ...
    expect(state).toEqual(
      ImmutableMap({
        test: ImmutableMap({
          abc: { negated: true, operator: OR, prop: 'abc', value: [] }
        })
      })
    );
  });
  ...
  it('SET_NEGATED existing change', () => {
    const initialState = ImmutableMap({
      test: ImmutableMap({
        abc: { negated: true, operator: OR, prop: 'abc', value: ['X'] }
      })
    });
    const state = reducer(initialState, {
      type: actionTypes.SET_NEGATED,
    ...
    expect(state).toEqual(
      ImmutableMap({
        test: ImmutableMap({
          abc: { negated: false, operator: OR, prop: 'abc', value: ['X'] }
        })
      })
    );
  });
  ...
  it('SET_NEGATED throws on wrong type', () => {
    ...
    ).toThrow(/^SET_NEGATED expects a boolean value. Received undefined$/);
    expect(() =>
      reducer(initialState, {
        type: actionTypes.SET_NEGATED,
        meta: { name: 'test', prop: 'abc', kind: kinds.value },
    ...
    ).toThrow(/^SET_NEGATED expects a boolean value. Received number$/);
  });
  ...
  it('SET_NEGATED throws on wrong kind', () => {
    ...
    expect(() =>
      reducer(initialState, {
        type: actionTypes.SET_NEGATED,
        meta: { name: 'test', prop: 'abc', kind: 'wrong' },
        payload: false
      })
    ).toThrow(/^unexpected kind: wrong$/);
  });
  ...
  it('SET_OPERATOR empty', () => {
    ...
    expect(state).toEqual(
      ImmutableMap({
        test: ImmutableMap({
          abc: { negated: false, operator: AND, prop: 'abc', value: [] }
        })
      })
    );
  });
  ...
  it('SET_OPERATOR existing no change', () => {
    const initialState = ImmutableMap({
      test: ImmutableMap({
        abc: { negated: true, operator: AND, prop: 'abc', value: [] }
      })
    });
    const state = reducer(initialState, {
      type: actionTypes.SET_OPERATOR,
